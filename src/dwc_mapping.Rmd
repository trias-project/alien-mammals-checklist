...
distribution <-
  ...
  mutate(
    ...
    dwc_countryCode = country_code,
    dwc_occurrenceStatus = occurrence_status,
    dwc_establishmentMeans = establishment_means,
    ...
    dwc_source = source,
    dwc_occurrenceRemarks = occurrence_remarks
  )
```
...
```{r}
species_profile %>%
  group_by(terrestrial, marine, freshwater, estuarine) %>%
  count()
```
...
species_profile %<>%
  mutate(
    dwc_taxonID = taxon_id,
    dwc_isMarine = marine,
    dwc_isFreshwater = freshwater,
    dwc_isTerrestrial = terrestrial #,
    # dwc_isEstuarine = estuarine
  )
```
...
```{r}
species_profile %>%
  select(dwc_isMarine, dwc_isFreshwater, dwc_isTerrestrial) %>%
  group_by_all() %>%
  summarize(records = n())
```
